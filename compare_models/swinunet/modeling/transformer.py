...
from modeling.cbam import ChannelGate

# 这里的代码参考的是vision transformer结构：
...
class ReShape(nn.Module):
    ...
    def __init__(self, inplanes=16, outplanes=512, BatchNorm=None):
        super(ReShape, self).__init__()
        self.conv = nn.Conv2d(inplanes, outplanes, kernel_size=3, stride=1, padding=1, bias=False)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sr_ratio > 1:
            global _H, _W
            x_ = x.permute(0, 2, 1).reshape(B, C, _H, _W)
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale   # @为矩阵乘法，只对最后两个维度进行相乘； scale为根号dk
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
                 ...
                 qk_scale=None, distilled=False, drop_ratio=0., attn_drop_ratio=0.,
                 drop_path_ratio=0., embed_layer=PatchEmbed, norm_layer=None,
                 act_layer=None, BatchNorm=None):
        """
        Args:
            ...
            depth (int): depth of transformer，重复堆叠次数
        ...
        self.ReShape = ReShape(16, 512, BatchNorm=BatchNorm)
        self.CA = ChannelGate(gate_channels=512)
        '''
    ...
    def forward(self, x):
        ...
        # x = [16, 32, 32]
        x = x.reshape(x.shape[0], (self.img_size // 2), (self.img_size // 2), -1).permute(0, 3, 1, 2).contiguous()
        x = self.ReShape(x)
        x = self.CA(x)
...
def build_transformer(num_classes: int = 1000, BatchNorm=None):
    """
