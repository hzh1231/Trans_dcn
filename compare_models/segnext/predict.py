...
from modeling.model import SegNext
...
def main():
    ...
    parser.add_argument('--backbone', type=str, default='bitformer',
                        choices=['resnet', 'xception', 'drn', 'mobilenet', 'mix_transformer', 'bitformer'],
                        help='backbone name (default: resnet)')
    parser.add_argument('--ckpt', type=str, default='./run/pascal/deeplab-bitformer/model_best.pth.tar',
                        help='saved model')
    ...
    '''
    model = SegNext(num_classes=args.num_classes, in_channnels=3, embed_dims=[64, 128, 320, 512],
                ffn_ratios=[8, 8, 4, 4], depths=[3, 3, 12, 3], num_stages=4,
                dec_outChannels=512, drop_path=float(2e-1),
                config=512)
