...
from modeling.backbone.vit import TIMMVisionTransformer

from functools import partial
from timm.models.layers import DropPath
import torch.utils.checkpoint as cp
from modeling.backbone.ms_deform_attn import MSDeformAttn
...
class ViTCoMer(TIMMVisionTransformer):
    ...
    def forward(self, x):
        ...
        output["out"] = [f1, f2, f3, f4]
        return output


def build_deit(pretrain=True, aux=False):
    model = ViTCoMer(
        patch_size=16,
        embed_dim=384,
        depth=12,
        ...
        window_size=[None] * 12, aux=aux)
    if pretrain:
        weights_dict = torch.load("./deit_small_patch16_224-cd65a155.pth", map_location='cpu')['model']
        weights_dict = {key.replace('blocks.', ''): value for key, value in weights_dict.items()}
        missing_keys, unexpected_keys = model.blocks.load_state_dict(weights_dict, strict=False)
...
if __name__ == "__main__":
    ...
    model = build_deit(pretrain=False, aux=True)
    model = model.cuda()
    input = input.cuda()
