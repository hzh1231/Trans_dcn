...
from modeling.trans_dcn import *
...
class Trainer(object):
    def __init__(self, args):
        ...
        # Define network
        model = Trans_dcn(num_classes=self.nclass,
        ...
        train_params = [{'params': model.get_1x_lr_params(), 'lr': args.lr},
                        {'params': model.get_10x_lr_params(), 'lr': args.lr * 10}]  # 原始为*10    
    ...
    def training(self, epoch, freeze_epochs):
        train_loss = 0.0
        self.model.train()
...
def main():
    parser = argparse.ArgumentParser(description="PyTorch Trans_dcn Training")
    parser.add_argument('--backbone', type=str, default='transformer',
                        choices=['resnet', 'xception', 'drn', 'mobilenet', 'transformer', 'swin_transformer'],
                        help='backbone name (default: transformer)')
    ...
    parser.add_argument('--freeze_epochs', type=int, default=0,
                        metavar='N', help='freeze backbone epochs (default:0)')
    ...
    if args.freeze_epochs > args.epochs:
        raise ValueError('Argument --freeze_epoch must smaller than --epochs')
    ...
    if args.checkname is None:
        args.checkname = 'trans_dcn-'+str(args.backbone)
    ...
    print('Freeze Epochs:', args.freeze_epochs)
    print('Total Epoches:', trainer.args.epochs)
    for epoch in range(trainer.args.start_epoch, trainer.args.epochs):
        if epoch == args.freeze_epochs:
            print('start training without freeze backbone!')
        trainer.training(epoch, args.freeze_epochs)
